...
- 使用生产者/消费者队列的方式生成通知 (这使通知更流畅)
- 修正了一处 `freechaf` 核心中的颜色转换错误 (感谢 XQuader)
- 加入了 `vice_xvic` 核心 (感谢 Salvacam)
- 由于用来计算的 3500 mAH 电池实际上是 2800 mAH 的，暂时删除了 3500 mAH 电池的 DTB 文件
- 从 Patreon 删除了 GarlicOS 1.3.4 版本的下载链接 (因为大多数 ports 已经兼容了 最新的 GarlicOS 版本)
- GarlicOS 版本号升级为 `1.4.7`

## 2023-04-11 (1.4.6):

- 应该被跳过的方向键的翻转事件不再触发 (例如，在坐标轴上 -1 到 1 之间的突变，这并没有意义)
- 在不需要遮罩的帧中禁用了硬件遮罩 (节约了 CPU 资源)
- 遮罩渲染线程以 1 FPS 的帧率进行渲染 (以节约 CPU 资源，一般来说对于通知已经够用了)
- 遮罩渲染线程固定使用第四个 CPU 核心 (考虑到 RetroArch 主要使用前两个核心，这可以提升 RetroArch 的性能)
- 将遮罩的帧缓冲移动到帧缓冲设备的末尾 (节约了 CPU 的消耗)
- 为 `PCSX ReARMed` 核心改进了一些性能 (约提升了 2~3 FPS)
- GarlicOS 菜单中的电量显示图更加精确 (例如, 81% 电量将显示为 80% 的图片，而非之前的 100%)
- GarlicOS 版本号升级为 `1.4.6`

## 2023-04-09 (1.4.5):

- 重新加入了 GarlicOS 中 SELECT 键的 CPU 控制功能 (--, -, normal, +, ++), 但它现在的含义不是让 CPU 使用固定的频率，而是设置一个最大 CPU 频率 (即, CPU 频率仍然是动态调整的, 但用户可以设置动态调整的上限)
- 进一步调整了调速器的细节，以修复 `ScummVM`, `DOS`, `NES`, `GBC` 等核心中出现的卡顿现象
- 为目前一致的所有电池增加了 DTB 文件 (请在 `misc/dtbs` 文件夹中找到对应型号的 DTB 文件, 并将其命名为 `kernel.dtb` 并覆盖 `MISC` 分区中的同名文件)
- GarlicOS 版本号升级为 `1.4.5`

## 2023-04-06 (1.4.4):

- 修正了新的驱动程序中的一处导致方向键出现大量意外事件的问题 (在 GarlicOS 菜单中造成了很奇怪的影响)
- 修正了一处在游戏退出时导致 RetroArch 全局配置被修改的问题
- 为新的 u-boot DTB 重新启用了低电量启动功能
- 优化了内核的电量百分比曲线 (基于 4 台 2600 mAH 和 3 台 2100 mAH 设备的测试数据。对于 2100 mAH 设备的用户，请手动删除 `kernel.dtb` 并将 `2100mAh.dtb` 替换之)
- 更新了 `PCSX ReARMed` 核心，增加了方向键映射到右摇杆的功能 (感谢 XQuader)
- 调整了调速器在 < 1 GHz 频率中的细节 (更流畅，且电源消耗更低)
- GarlicOS 版本号升级为 `1.4.4`

## 2023-04-05 (1.4.3):

- 由于 GDB 日志影响了一些核心的表现，将其设置为可选项 (主要是 `ScummVM` 和 `mGBA` 核心。可以在 `misc` 分区中手动创建一个 `enableGDB` 文件启用它)
- 将 u-boot 升级到最新的零售版本 (修正了新的 2600 mAH PCB 版本的机型在启动画面卡住的问题)
- 修改内核使之与零售版本匹配 (修正了新的 2600 mAH PCB 版本的机型在启动画面卡住的问题)
- 在首次启动时，MicroSD 卡的镜像不再安装 u-boot 的更新 (因为 MicroSD 卡的版本总是最新的)
- 修正了一处在 RetroArch 的 CPU 控制菜单中无法选择 "性能" 的问题
- 修正了一处 RetroArch 的 CPU 控制菜单中导致电量消耗过大的问题
- 除了用户空间之外，暂时忽略了 RetroArch 的 CPU 频率设置 (因为它会触发一个内核问题)
- 将更新了的调速器设置为新的默认值 (目前工作良好)
- 回滚了之前的 CPU 核心设置 (请手动删除 `CFW/retroarch/.retroarch/config` 文件夹)
- GarlicOS 版本号升级为 `1.4.3`

## 2023-04-04 (1.4.2):

- 修正了休眠和恢复功能
- 修正了一处在开关线程渲染时崩溃的问题
- 修正了一处在切换普通和线程渲染时导致遮罩和通知出错的问题
- GarlicOS 版本号升级为 `1.4.2`

## 2023-04-03 (1.4.1):

- 修正了方向键零点漂移的问题
- 遮罩渲染器在 RetroArch 退出时会正确地清空
- 修正了无法关闭 RetroArch 遮罩的问题
- 现在 `ZX Spectrum` 核心 (fuse) 会自动裁剪黑边框
- 修正了 `mGBA` 核心导致 RetroArch 的视频设置中出现全屏 / 窗口化选项的问题
- 现在屏幕截图尽可能地异步执行 (这使快速存档更加流畅。自动存档的截图仍然是同步执行的)
- 修正了在 GarlicOS 菜单中 CPU 在空闲时仍在最高频率运行的问题
- 修正了在 GarlicOS 菜单中刷新率降低的问题
- 修正了在 GarlicOS 菜单中，按下电源键不关机的问题
- 在 GarlicOS 和 RetroArch 中，短按电源键都可以关机
- 为便于用户汇报问题，开启了 GDB 文件日志 (`CFW/retroarch/.retroarch/logs/gdb.log` 文件, 在 `retroarch.log` 文件旁边)
- 修改了处理 RetroArch 配置文件的细节 (配置自动合并到 `retroarch.cfg` 文件中)
- 修正了 RetroArch CPU 控制的问题 (修正了游戏退出时的问题)
- 增加了自动开始脚本的支持 (将脚本文件放在 `CFW/autostart` 文件夹下，在开机时会自动运行)
- 由于部分用户的问题，将支持 UHS 的 TF2 内核设置为可选项，并放在 `kernel-tf2uhs.dtb` 文件中 (我自己必须用这个才能运行, 用户可以自行选择需要的内核，将其改名为 `kernel.dtb` 并重启设备)
- GarlicOS 版本号升级为 `1.4.1`

## 2023-04-01 (1.4.0):

- 修正了 RetroArch 菜单中的通知
- 修正了 "主机" 菜单中的一处页面滚动错误
- 移除了 GarlicOS 的 CPU 控制 (详见下一条)
- 增加了 RetroArch CPU 控制 (这使我们可以按核心，甚至按游戏调整 CPU 的配置)
- 没有额外配置过的核心暂时将以最高 CPU 速度运行 (以免影响用户体验)
- 从 RetroArch 中移除了游戏模式的控制 (因为 RG35XX 不支持该模式)
- 将 `Gambatte` 核心配置为在 504MHz 模式下运行
- 将 `mGBA` 核心配置为在 1.3GHz 模式下运行 (黄金太阳)
- 将 `FCEUmm` 核心配置在 720MHz 模式下运行
- 将 `Supafaust` 核心配置在 1.3GHz 模式下运行 (超级马里奥 RPG)
- `Supafaust` 核心的渲染器在独立的线程中运行 (SuperFX)
- 在 GarlicOS 中，Select 键暂时和开始键起相同的作用
- 从 skin 目录中移除了超频 / 降频的图片 (因为不再需要了)
- GarlicOS 版本号升级为 `1.4.0`

## 2023-04-01 (1.3.9)

- 在自动映射的 RetroArch 核心中, MENU 和 音量+ 键不再触发 L3 / R3 (修正了在 `FCEUmm` 核心中不必要的重复映射。如有需要，也可以使用 `RG35XX Gamepad` 自动映射文件达到上一个版本的效果)
- 硬件加速的遮罩渲染逻辑现在是多线程的、双重缓冲的，并且有帧数上限 (避免了闪烁的通知现象)
- 硬件加速的遮罩渲染逻辑修正支持 2 个以上的 Z-layer (这是下一条所必须的)
- 加入了系统边框的支持 (也称为 RetroArch 遮罩), 可在 RetroArch 中进行设置 (此时仅支持 `640x480` 像素的单一图片的遮罩)
- 加入了若干兼容 RG35XX 的系统边框 (目前包括 FC, SFC, GB, GBC & GBA。如有需要，请按 `CFW/retroarch/.retroarch/overlay` 文件夹中的模板自由修改。如需打包到系统中，请联系我)
- 热键说明的遮罩信息可以在 RetroArch 的遮罩设置中关闭
- 游戏截图现在强制同步地写入到磁盘 (以确保自动存档的截图能在退出 RetroArch 之前完成保存)
- TF2 卡槽现在支持 UHS 卡
- 修正了一处导致方向键卡住的问题
- GarlicOS 版本号升级为 `1.3.9`
...
- GarlicOS 版本号升级为 `1.3.5`

## 2023-03-24 (1.3.4)

- 在启动时自动创建存档和截图的目录 (以克服某些 `retroarch.cfg` 的限制)
- 在独立的线程中驱动震动马达 (这将修正 PlayStation 和 GBA 游戏中的卡顿现象)
- 调整了震动马达的震动强度公式，以提供更好的震动效果 (例如: `GT 赛车 2`)
- 退出 RetroArch 时将重置震动马达
- GarlicOS 版本号升级为 `1.3.4`

## 2023-03-22 (1.3.3)

- 更新了 `scummvm` 核心，加入了屏幕键盘和手柄映射修改的支持 (感谢 XQuader)
- 打包了 ScummVM 的 "BIOS" 文件 (OnionOS 这样做似乎没什么问题)
- 清理了一些文件 (例如, Cheats 文件夹， `retroarch.log` 文件和多余的 mGBA config 文件夹)
- 将压缩包按 200 MB 分卷 (因为超过了 Patreon 的文件大小限制)
- GarlicOS 版本号升级为 `1.3.3`
...
- 由于保守 CPU 调速器工作不太理想, 重新启用了手动超频/降频开关 (GarlicOS 菜单将总是使用 700MHz, 手动开关只影响游戏中的 CPU 频率)
...
- 使用更保守的 CPU 调速器, 使 CPU 能够更平滑地在不同状态间切换 (在减少卡顿的情况下提升续航)
- 移除了手动超频 / 降频的功能，因为 CPU 调速器工作的不错
- 修正了一处游戏图片的路径错误
